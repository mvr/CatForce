...
LinearTransform LTCompose(const LinearTransform A, const LinearTransform B) {
  ...
  if (A >= 4 && B >= 4) { // both reflections => same as rotating by 2x angle
                          // between lines.
    return static_cast<LinearTransform>((A - B + 4) % 4);
  }
  if (A < 4 &&
      B >= 4) { // reflection first => rotate the line by half of the angle
    return static_cast<LinearTransform>((B + A) % 4 + 4);
  }

  if (A >= 4 && B < 4) { // reflection second => same as above but direction of
                         // rotation flips
    return static_cast<LinearTransform>((A - B) % 4 + 4);
  }
}
...
std::pair<int, int> ApplyLinearTransform(const LinearTransform T,
...
public:
  ...
  AffineTransform(LinearTransform T, int x0, int y0)
      : matrix(T), transl{x0, y0} {}
  AffineTransform(LinearTransform T) : matrix(T), transl{0, 0} {}
  AffineTransform(int x0, int y0)
      : matrix(LinearTransform::Rotate0), transl{x0, y0} {}
  ...
  std::pair<int, int> ActOn(const std::pair<int, int> vec) const { // A*x + b.
    return std::make_pair(
        ApplyLinearTransform(matrix, vec).first + transl.first,
        ApplyLinearTransform(matrix, vec).second + transl.second);
  }

  AffineTransform Compose(const AffineTransform other)
      const { // A1(A2x+b2)+b1 = (A1 A2)x + (A1 b2 + b1)
    LinearTransform newMatrix = LTCompose(matrix, other.matrix);
    std::pair<int, int> matOfOtherTransl =
        ApplyLinearTransform(matrix, other.transl);
  ...
  AffineTransform Inverse() const {
    ...
    std::pair<int, int> negOfTransl =
        ApplyLinearTransform(inverseMat, transl); // this is A^{-1} b

    return AffineTransform(inverseMat, -1 * negOfTransl.first,
                           -1 * negOfTransl.second);
  }

  bool IsOrientationPreserving() { return matrix < 4; }

  /*void Print() {
    for( auto num : matrix ) {
      std::cout << std::to_string( num ) << " ";
    }
    std::cout << std::to_string(transl[0]) << " " << std::to_string(transl[1])
  << std::endl;
  }*/

  /*AffineTransform ShiftedBy(const std::array<int,2> & otherTransl){
    return AffineTransform(matrix[0], matrix[1], matrix[2], matrix[3], transl[0]
  + otherTransl[0], trans[1]+otherTransl[1]);
  }*/
};
...
void LifeState::Transform(AffineTransform transform) {
  ...
  if (transform.matrix == 3 || transform.matrix == 5) {
    ...
    Transpose(false);
    transform =
        transform.Compose(AffineTransform(LinearTransform::FlipAcrossYEqX));

  } else if (transform.matrix == 1 || transform.matrix == 7) {
    // rotate 90 and flipYEqNegXP1 become ReflectY and Identity.
    Transpose(true); // this is has [0, -1, -1, 0] as matrix, [-1, -1] for
                     // translation.
    Move(1, 1);
    transform = transform.Compose(
        AffineTransform(LinearTransform::FlipAcrossYEqNegXP1));
  }

  if (transform.matrix == 4 || transform.matrix == 2) {
    FlipAcrossX();
    transform =
        transform.Compose(AffineTransform(LinearTransform::FlipAcrossX));
  }

  if (transform.matrix == 6) {
    FlipAcrossY();
    transform =
        transform.Compose(AffineTransform(LinearTransform::FlipAcrossY));
  }
